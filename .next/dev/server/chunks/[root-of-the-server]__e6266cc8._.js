...
const PURCHASE_FROM = process.env.PURCHASE_EMAIL_FROM || 'Mimosa Spa Retreat <compras@mimosaretreat.com>';
const BOOKING_FROM = process.env.BOOKING_EMAIL_FROM || 'Mimosa Spa Retreat <citas@mimosaretreat.com>';
const GIFT_FROM = process.env.GIFTCARD_EMAIL_FROM || 'Mimosa Spa Retreat <regalos@mimosaretreat.com>';
async function sendEmail(input) {
    ...
    try {
        const res = await fetch('https://api.resend.com/emails', {
            ...
            body: JSON.stringify({
                from: input.kind === 'booking' ? BOOKING_FROM : input.kind === 'gift' ? GIFT_FROM : PURCHASE_FROM,
                to: [
                    input.to
                ],
...
"[project]/src/lib/email/templates/booking.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
function bookingConfirmationEmail(d) {
    const rows = [
        [
            'Sede',
            d.locationName
    ...
    if (d.services?.length) rows.push([
        'Servicios',
        d.services.join(', ')
    ]);
    if (d.therapistName) rows.push([
        'Terapeuta',
        d.therapistName
    ]);
    return {
        subject: `Tu cita en Mimosa Spa está confirmada — ${d.date}`,
        html: shell(`¡Cita confirmada, ${d.clientName}!`, `<p style="font-family:Arial,sans-serif;font-size:14px;color:#54514D;line-height:1.6;margin:0;">
...
function bookingReminderEmail(d) {
    return {
        subject: `Recordatorio: tu cita en Mimosa Spa es mañana — ${d.date}`,
        html: shell(`Nos vemos mañana, ${d.clientName} 💛`, `<p style="font-family:Arial,sans-serif;font-size:14px;color:#54514D;line-height:1.6;margin:0;">
...
function bookingCancellationEmail(d) {
    return {
        subject: `Tu cita en Mimosa Spa fue cancelada — ${d.date}`,
        html: shell(`Hola ${d.clientName}`, `<p style="font-family:Arial,sans-serif;font-size:14px;color:#54514D;line-height:1.6;margin:0;">
...
"[project]/src/app/api/cron/sync-appointments/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
var __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$email$2f$templates$2f$booking$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__ = __turbopack_context__.i("[project]/src/lib/email/templates/booking.ts [app-route] (ecmascript)");
;
;
;
;
...
function normalizePhoneForWati(phone) {
    const digits = (phone || '').replace(/\D/g, '');
...
async function GET(request) {
    ...
    const liveAppointmentsGlobal = [];
    const liveIds = new Set();
    const okLocations = new Set();
    ...
    // single dataset powers cancellation detection AND staff-booking ingestion.
    {
        let allLocationIds = [];
        try {
            const locs = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$booking$2f$mindbody$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["getLocations"])();
        ...
        for (const locationId of allLocationIds){
            try {
                ...
                const PAGE = 200;
                while(true){
                    const { appointments, pagination } = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$booking$2f$mindbody$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["getStaffAppointments"])({
                        locationId,
                        startDate: winStart,
                        endDate: winEnd,
                        limit: PAGE,
                    ...
                    for (const apt of appointments){
                        liveIds.add(apt.Id);
                        liveAppointmentsGlobal.push(apt);
                    }
                    const total = pagination?.TotalResults ?? appointments.length;
                    offset += PAGE;
    ...
    const { data: upcoming } = await supabase.from('bookings').select('id, client_name, client_phone, client_email, location_id, location_name, appointment_start, mindbody_appointment_ids, services').eq('status', 'confirmed').gte('appointment_start', nowIso).lte('appointment_start', horizon).not('mindbody_appointment_ids', 'is', null);
    if (upcoming && upcoming.length > 0) {
        for (const booking of upcoming){
            const firstId = booking.mindbody_appointment_ids?.[0];
            if (!firstId) continue;
            if (!okLocations.has(booking.location_id)) continue;
            ...
            const { data: flipped } = await supabase.from('bookings').update({
                status: 'cancelled',
                mindbody_status: 'Cancelled',
                status_synced_at: new Date().toISOString()
            }).eq('id', booking.id).eq('status', 'confirmed').select('id');
            ...
            const raw = booking.appointment_start;
            ...
            const firstServiceId = booking.services?.[0]?.sessionTypeId;
            const reagendarUrl = firstServiceId ? `https://www.mimosaretreat.com/es/reservar?serviceId=${firstServiceId}` : 'https://www.mimosaretreat.com/es/reservar';
            ...
            if ((0, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$booking$2f$wati$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["isWatiConfigured"])() && booking.client_phone && booking.client_name) {
                const result = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$booking$2f$wati$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["sendBookingCancellation"])({
            ...
            if (!noticeSent && (0, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$email$2f$resend$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["isEmailConfigured"])() && booking.client_email) {
                const mail = (0, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$email$2f$templates$2f$booking$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["bookingCancellationEmail"])({
    ...
    let staffConfirmationsSent = 0;
    {
        ...
        const { data: knownRows } = await supabase.from('bookings').select('mindbody_appointment_ids').gte('appointment_start', new Date(Date.now() - 24 * 60 * 60 * 1000).toISOString());
        const knownIds = new Set();
        for (const r of knownRows || []){
            for (const id of r.mindbody_appointment_ids || [])knownIds.add(id);
        }
        ...
        const { count: baselineCount } = await supabase.from('bookings').select('id', {
            count: 'exact',
            head: true
        }).like('confirmation_number', 'MB-%');
        const isFirstIngest = (baselineCount ?? 0) === 0;
        const fresh = liveAppointmentsGlobal.filter((a)=>a.ClientId && Number(a.ClientId) > 0 && !knownIds.has(a.Id) && (a.Status === 'Booked' || a.Status === 'Confirmed') && new Date(`${a.StartDateTime}-05:00`).getTime() > Date.now());
        // Session type names for the services list
        const sessionTypeNames = new Map();
        if (fresh.length > 0) {
            try {
                for (const st of (await (0, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$booking$2f$mindbody$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["getSessionTypes"])(false)))sessionTypeNames.set(st.Id, st.Name);
            } catch (err) {
        ...
        for (const group of visits.values()){
            ...
            let clientName = '';
            try {
                const mbClient = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$booking$2f$mindbody$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["getClientWithCustomFields"])(String(clientId));
            ...
            const durationOf = (a)=>a.Duration || Math.max(0, Math.round((new Date(a.EndDateTime).getTime() - new Date(a.StartDateTime).getTime()) / 60000));
            ...
            const therapistName = first.Staff && (`${first.Staff.FirstName || ''} ${first.Staff.LastName || ''}`.trim() || first.Staff.DisplayName) || null;
            ...
            let confirmed = false;
            if ((0, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$booking$2f$wati$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["isWatiConfigured"])() && phone && clientName) {
                const r = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$booking$2f$wati$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["sendBookingConfirmation"])({
                    clientName,
                    clientPhone: phone,
                    ...
                    totalDuration,
                    therapistName: therapistName || 'Por asignar'
                });
                if (r.result) {
                    confirmed = true;
                    ...
                    await supabase.from('bookings').update({
                        whatsapp_sent: true
                    }).eq('confirmation_number', `MB-${first.Id}`);
                }
            }
            if (!confirmed && (0, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$email$2f$resend$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["isEmailConfigured"])() && email) {
    ...
    const { data: bookings, error } = await supabase.from('bookings').select('id, mindbody_appointment_ids, status, appointment_start, location_id').eq('status', 'confirmed').lt('appointment_start', now).not('mindbody_appointment_ids', 'is', null).order('appointment_start', {
        ascending: true
    });
    if (error) {
    ...
    const oldestDate = new Date(bookings[0].appointment_start);
    const windowEnd = new Date(oldestDate);
    windowEnd.setDate(windowEnd.getDate() + WINDOW_DAYS);
    // Bookings within this window
    const windowEnd_iso = windowEnd.toISOString();
    const windowBookings = bookings.filter((b)=>b.appointment_start <= windowEnd_iso);
    const targetIds = new Set(windowBookings.map((b)=>b.mindbody_appointment_ids?.[0]).filter(Boolean));
    const startDate = oldestDate.toISOString().split('T')[0];
    const endDate = (windowEnd < new Date() ? windowEnd : new Date()).toISOString().split('T')[0];
    ...
    const fullyFetchedLocations = new Set();
    ...
    function mapStatus(mindbodyStatus) {
    ...
    for (const booking of windowBookings){
        ...
        if (!fullyFetchedLocations.has(booking.location_id)) continue;
...
//# sourceMappingURL=%5Broot-of-the-server%5D__e6266cc8._.js.map