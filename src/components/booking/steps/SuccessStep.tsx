  ...
  return (
    <div className="success-step text-center">
      ...
      <motion.div
        ...
        className="mb-8"
      >
        <div className="w-24 h-24 bg-gradient-to-br from-green-400 to-green-500 rounded-full 
      ...
      <motion.div
        ...
        className="bg-white border-2 border-beige-200 rounded-2xl p-6 text-left max-w-md mx-auto mb-8"
      >
        {/* Promotion Badge */}
        {activePromotion && (
          <div className="flex items-center gap-2 mb-4 bg-gold/10 px-3 py-2 rounded-lg">
            <Star className="w-5 h-5 text-gold" />
            <span className="font-semibold text-dark">{activePromotion.title_es}</span>
          </div>
        )}
        ...
        <div className="flex items-start gap-3 mb-4">
          ...
          <div>
            <p className="font-semibold text-dark">{selectedLocation?.Name}</p>
        ...
        <div className="border-t border-beige-200 pt-4 mt-4">
          <p className="text-sm font-semibold text-warm-gray mb-2">SERVICIOS:</p>
          <ul className="space-y-1">
            {selectedServices.map(service => (
              <li key={service.Id} className="text-sm text-dark flex justify-between">
                <span>{service.Name}</span>
                <span className="text-warm-gray">{service.Duration} min</span>
              </li>
            ))}
            {selectedAddons.map(addon => (
              <li key={addon.Id} className="text-sm text-gold-600 flex justify-between">
        ...
        {pricing && (
          <div className="border-t border-beige-200 pt-4 mt-4">
            <div className="flex justify-between items-center">
              <span className="font-semibold text-dark">Total a pagar:</span>
      ...
      <motion.div
        ...
        className="bg-amber-50 border border-amber-200 rounded-xl p-4 max-w-md mx-auto mb-8 text-left"
      >
        ...
        <ul className="text-sm text-amber-700 space-y-1">
          ...
          <li>• Para cancelar, contáctanos con 24 horas de anticipación</li>
        </ul>
      </motion.div>
      ...
      >
        <Link
          href="/"
          className="flex items-center gap-2 px-6 py-3 bg-beige-100 text-dark 
                   font-medium rounded-xl hover:bg-beige-200 transition-all"
        >
