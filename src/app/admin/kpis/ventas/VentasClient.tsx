...
import {
  ...
  deltaPct, formatDateEs, money, money2, moneyCompact,
} from '../shared'
...
export function VentasClient() {
  ...
  return (
    <div className="max-w-xl">
      <div className="mb-5">
        <h1 className="text-3xl font-display font-semibold text-dark">Reporte de Ventas</h1>
        <p className="text-sm text-warm-gray mt-1">Ventas netas por día, sin ITBMS ni propinas · comparado con el año pasado</p>
      </div>
      ...
      <div className="sticky top-14 lg:top-0 z-20 -mx-2 px-2 pt-2 pb-3 mb-2 bg-cream/95 backdrop-blur-sm border-b border-beige-300">
        <div className="flex flex-wrap gap-2 mb-2" role="group" aria-label="Período">
          ...
          >
            ...
            {preset === 'custom' ? `${range.start} → ${range.end}` : 'Rango'}
          </button>
        </div>
      ...
      {loading || !data ? (
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-2xl border border-gold-200 bg-gold-50 p-4">
            <Label>{gcMode ? 'Gift cards redimidas · comparativo' : 'Ventas netas · comparativo'}</Label>
            ...
            <p className="text-xs text-warm-gray mt-1">
              {data.totals.lyNet > 0
                ? `${money(data.totals.lyNet)} en las mismas fechas de ${lyYear}`
                : `sin datos de ${lyYear} para comparar`}
            </p>
            {data.lyFull && data.lyFull.net > 0 && (
              <p className="text-xs text-warm-gray mt-0.5">
                Meta — {data.lyFull.kind === 'year' ? `${lyYear} completo` : `mes completo ${lyYear}`}: <b className="text-dark">{money(data.lyFull.net)}</b>
              </p>
            )}
            <p className="text-[11px] text-warm-gray mt-3">
              {data.range.start} → {data.range.end} vs {data.lyRange.start} → {data.lyRange.end}
              {data.series.unit === 'day' ? ' · por día' : ' · por mes'}
            </p>
          ...
          <CardBox className="p-0 overflow-hidden">
            {data.days.length === 0 && <p className="text-sm text-warm-gray p-4">Sin ventas en el período.</p>}
            {data.days.map(d => {
              ...
              return (
                <div key={d.date} className="border-b border-beige-200 last:border-b-0">
                  ...
                  >
                    <span className="text-sm text-dark capitalize">{formatDateEs(d.date)}</span>
                    <span className="flex items-center gap-3 shrink-0">
                      <span className="flex flex-col items-end gap-0.5">
                        <span className="flex items-baseline gap-3">
                          <span className="text-xs text-warm-gray tabular-nums">{d.visits} {gcMode ? 'usos' : 'visitas'}</span>
                  ...
                  {open && (
                    <div className="px-4 pb-3 bg-beige-100/60">
                      ...
                      {state?.status === 'error' && (
                        <p className="text-xs text-red-700 py-2">No se pudieron cargar las transacciones.</p>
                      )}
                      {state?.status === 'ready' && (
                        <div className="divide-y divide-dashed divide-beige-400">
                          {state.transactions.length === 0 && (
                            <p className="text-xs text-warm-gray py-2">Sin transacciones este día.</p>
                          )}
                          {state.transactions.map(t => {
                            const shown = t.items.filter(it => it.bucket !== 'tip')
                            const tip = t.items.filter(it => it.bucket === 'tip').reduce((s, it) => s + it.net, 0)
                            return (
                              <div key={t.saleId} className="py-2.5">
                                <div className="flex items-baseline justify-between gap-3">
                                  <span className="text-sm text-dark">
                                    <span className="tabular-nums text-warm-gray">{t.time}</span>{' '}
                                    {t.clientName ?? (t.clientId ? `Cliente ${t.clientId}` : 'Sin cliente')}
                                  </span>
                                  <b className="text-sm text-dark tabular-nums shrink-0">{money2(t.net)}</b>
                                </div>
                                <p className="text-xs text-warm-gray mt-0.5">
                                  #{t.saleId} · {shown.map(it => `${it.returned ? '(dev) ' : ''}${it.description}`).join(' · ') || '—'}
                                  {t.paymentTypes.length > 0 && <> · {t.paymentTypes.join(', ')}</>}
                                  {!gcMode && t.gcPaid > 0 && <> · pagado con gift card {money2(t.gcPaid)}</>}
                                  {tip > 0 && <> · propina {money2(tip)}</>}
                                </p>
                              </div>
                            )
                          })}
            ...
            {data.days.length > 0 && (
              <div className="flex items-center justify-between gap-3 px-4 py-3.5 bg-dark text-cream">
                <span className="text-sm font-bold">Total del período</span>
                <span className="flex flex-col items-end gap-0.5">
                  <span className="flex items-baseline gap-4">
                    <span className="text-xs text-beige-300 tabular-nums">{gcMode ? `${data.totals.saleCount.toLocaleString('en-US')} usos` : `${data.totals.visits.toLocaleString('en-US')} visitas · ${data.totals.saleCount.toLocaleString('en-US')} ventas`}</span>
                    <b className="text-base tabular-nums">{money2(data.totals.net)}</b>
                  </span>
                  <span className="text-[10px] font-bold tabular-nums text-gold">
                    {lyYear}: {data.totals.lyNet > 0 ? money(data.totals.lyNet) : '—'} (mismas fechas)
                  </span>
                </span>
              </div>
            )}
          ...
          <p className="text-center text-xs text-warm-gray pb-4">
            ...
            {' · hoy se actualiza al entrar y cada 5 min'}
          </p>
        </div>
      )}
    </div>
