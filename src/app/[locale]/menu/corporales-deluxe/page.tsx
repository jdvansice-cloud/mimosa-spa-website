...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const t = await getTranslations({ locale, namespace: 'menuPages.corporalesDeluxe' })
  return {
    title: t('title'),
    description: t('intro'),
  }
...
export default async function CorporalesDeluxePage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params
  const t = await getTranslations({ locale, namespace: 'menuPages.corporalesDeluxe' })
  const tCommon = await getTranslations({ locale, namespace: 'common' })
  const bannerImage = await getSiteImage('menu_corporales_deluxe_banner')
