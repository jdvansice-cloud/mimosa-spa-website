...
export const TOOLS: Anthropic.Tool[] = [
  ...
  tool('book', 'Crea la cita en Mindbody. Requiere customer_confirmation con el texto exacto con el que el cliente dijo que sí al resumen.', { sucursal: suc, date: { type: 'string' }, time: { type: 'string', description: 'HH:mm 24h' }, service_ids: { type: 'array', items: { type: 'integer' } }, addon_ids: { type: 'array', items: { type: 'integer' }, description: 'Extras a agregar; lista vacía si no lleva' }, staff_id: { type: 'integer', description: 'Id de la terapeuta pedida por el cliente; 0 = cualquiera disponible' }, people: { type: 'integer', enum: [1, 2] }, promo_title: { type: 'string', description: 'Título exacto de la promoción que escogió el cliente; cadena vacía si no es una promoción' }, promo_service_ids: { type: 'array', items: { type: 'integer' }, description: 'Los mindbody_service_ids que incluye esa promoción; lista vacía si no es una promoción' }, customer_confirmation: { type: 'string' } }, ['sucursal', 'date', 'time', 'service_ids', 'addon_ids', 'staff_id', 'people', 'promo_title', 'promo_service_ids', 'customer_confirmation']),
