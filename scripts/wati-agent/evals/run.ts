...
for (const c of picked) {
  ...
  for (const [ti, turn] of c.turns.entries()) {
    ...
    const g = await anthropic.messages.create({
      ...
      max_tokens: 200,
      system: 'Califica de 1 a 5 qué tanto la respuesta A suena como la recepcionista real B (tono, largo, calidez, formato). Responde solo JSON {"score":n,"why":"..."}',
      messages: [{ role: 'user', content: `Cliente: ${turn.customer.join(' / ')}\n\nA (Camila):\n${camila || '(handoff)'}\n\nB (humana):\n${turn.staff.join('\n')}` }],
    })
    ...
    try {
      ...
      score = start >= 0 && end >= start ? Number(JSON.parse(gt.slice(start, end + 1)).score) || 0 : 0
    } catch {
      score = 0
    }
