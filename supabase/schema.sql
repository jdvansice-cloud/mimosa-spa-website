-- ===========================================
-- MIMOSA SPA WEBSITE - COMPLETE DATABASE SCHEMA
-- Version: 1.1.0
-- Generated: 2026-01-16
-- ===========================================
-- This is a combined schema file that includes all migrations.
...
CREATE TABLE IF NOT EXISTS public.profiles (
  ...
  mindbody_client_id BIGINT,
  created_at TIMESTAMPTZ DEFAULT NOW(),
  updated_at TIMESTAMPTZ DEFAULT NOW()
);

-- Create index for faster Mindbody client ID lookups
...
-- STORAGE BUCKETS
-- ===========================================
-- Note: Run these in Supabase Dashboard > Storage or SQL Editor
...
-- Storage policies
CREATE POLICY "Public read access for promotions" ON storage.objects
...
CREATE POLICY "Admins can upload to promotions" ON storage.objects
...
CREATE POLICY "Admins can delete from promotions" ON storage.objects
  FOR DELETE USING (
    bucket_id = 'promotions' AND
    EXISTS (SELECT 1 FROM public.profiles WHERE profiles.id = auth.uid() AND profiles.role = 'admin')
  );
...
CREATE POLICY "Admins can delete from assets" ON storage.objects
  FOR DELETE USING (
    bucket_id = 'assets' AND
    EXISTS (SELECT 1 FROM public.profiles WHERE profiles.id = auth.uid() AND profiles.role = 'admin')
  );
